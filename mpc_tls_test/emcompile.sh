...
echo "compile client"
em++ -s WASM=1 -ggdb3 client.cpp  os.cpp -L${EMOPENSSL_PATH}/lib -L${EMOTLS_PATH}/mpc_tls -L${EMEMP_PATH}/lib -lwebsocket.js -sPROXY_POSIX_SOCKETS -sUSE_PTHREADS -sPROXY_TO_PTHREAD -sFORCE_FILESYSTEM -sMAIN_MODULE -lotls -lemp-tool -lemp-zk -lssl -lcrypto -ldl -lrt -pthread -o client.html -I../ -I${EMOPENSSL_PATH}/include ${CIRCUIT_FILE1} ${CIRCUIT_FILE2} ${CIRCUIT_FILE3} ${CIRCUIT_FILE6} -s TOTAL_STACK=128MB -s INITIAL_MEMORY=256MB -s ALLOW_MEMORY_GROWTH=1
echo "compile pado"
