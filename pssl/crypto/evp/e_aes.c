                              ...
                              const unsigned char *in, size_t len)
{
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, ctx->encrypt ? EVP_CTRL_GCM_IV_GEN
        ...
        goto err;
    if (need_mpc()) {
        unsigned char* aad = ctx->buf;
        size_t aad_len = gctx->tls_aad_len;
        ...
        if (ctx->encrypt) {
            ...
            //     printf("%2x ", in[i]);
            // printf("\n");
            size_t msg_len = len - EVP_GCM_TLS_EXPLICIT_IV_LEN - EVP_GCM_TLS_TAG_LEN;
            unsigned char* msg = OPENSSL_malloc(msg_len);
            memcpy(msg, in + EVP_GCM_TLS_EXPLICIT_IV_LEN, msg_len);

            size_t aad_len = gctx->tls_aad_len;
            transfer_hash_tls((unsigned char*)&aad_len, sizeof(size_t));
            transfer_hash_tls(aad, aad_len);

            transfer_hash_tls((unsigned char*)&msg_len, sizeof(size_t));
            if (transfer_flag) {
                transfer_hash_tls(msg, msg_len);
            }

            size_t iv_len = EVP_GCM_TLS_EXPLICIT_IV_LEN;
            transfer_hash_tls((unsigned char*)&iv_len, sizeof(size_t));
            transfer_hash_tls(out, iv_len);

            unsigned char* ctxt = out + EVP_GCM_TLS_EXPLICIT_IV_LEN;
            unsigned char* tag = out + len - EVP_GCM_TLS_TAG_LEN;
            enc_aesgcm_tls(ctxt, tag, msg, msg_len, aad, aad_len, out, EVP_GCM_TLS_EXPLICIT_IV_LEN, transfer_flag);
            OPENSSL_free(msg);
            return len;
        }
        else {
            ...
            //     printf("%2x ", in[i]);
            // printf("\n");
            size_t ctxt_len = len - EVP_GCM_TLS_EXPLICIT_IV_LEN - EVP_GCM_TLS_TAG_LEN;
            unsigned char* ctxt = OPENSSL_malloc(ctxt_len);
            memcpy(ctxt, in + EVP_GCM_TLS_EXPLICIT_IV_LEN, ctxt_len);

            unsigned char* msg = out + EVP_GCM_TLS_EXPLICIT_IV_LEN;
            unsigned char* tag = out + len - EVP_GCM_TLS_TAG_LEN;
            ...
            transfer_hash_tls((unsigned char*)&ctxt_len, sizeof(size_t));
            if (transfer_flag) {
                transfer_hash_tls(ctxt, ctxt_len);
            }
            ...
            transfer_hash_tls((unsigned char*)&tag_len, sizeof(size_t));
            transfer_hash_tls(tag, tag_len);

            dec_aesgcm_tls(msg, ctxt, ctxt_len, tag, aad, aad_len, out, EVP_GCM_TLS_EXPLICIT_IV_LEN, transfer_flag);
            OPENSSL_free(ctxt);
            return len;
        }
    }
    // printf("iv[%d]", EVP_GCM_TLS_EXPLICIT_IV_LEN);
    // for (size_t i = 0; i < EVP_GCM_TLS_EXPLICIT_IV_LEN; i++) {
    //     printf("%2x ", out[i]);
    // }
    // printf("\n");

    unsigned char* enc_key = gctx->gcm.key;
    // printf("key[%d]", 16);
