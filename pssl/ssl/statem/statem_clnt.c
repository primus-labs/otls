...
int tls_construct_client_hello(SSL *s, WPACKET *pkt)
{
    ...
    if (need_mpc()) {
        transfer_hash_tls(p, SSL3_RANDOM_SIZE);
    }
    /* Session ID */
...
MSG_PROCESS_RETURN tls_process_server_hello(SSL *s, PACKET *pkt)
{
    ...
    if (s->version == TLS1_3_VERSION
        ...
        if (need_mpc()) {
            transfer_hash_tls(s->s3->server_random, SSL3_RANDOM_SIZE);
        }
    }
...
{
    // printf("begin process server key exchange\n");
    long alg_k;
    EVP_PKEY *pkey = NULL;
    ...
    // printf("end process server key exchange\n");
...
{
#ifndef OPENSSL_NO_EC
    // printf("begin construct cke ecdhe\n");
    ...
    if (need_mpc()) {
        EC_POINT* pub_key = get_pub_key_tls(skey);

        EC_GROUP* group = EC_GROUP_new_by_curve_name(NID_X9_62_prime256v1);
        EC_POINT* client_key = EC_POINT_new(group);
        get_pms_tls(client_key, pub_key);

        EC_POINT* old = set_pub_key_tls(ckey, client_key);
        encoded_pt_len = EVP_PKEY_get1_tls_encodedpoint(ckey, &encodedPoint);
        set_pub_key_tls(ckey, old);

        EC_POINT_free(client_key);
        EC_GROUP_free(group);
    }
    else {
        encoded_pt_len = EVP_PKEY_get1_tls_encodedpoint(ckey, &encodedPoint);
    ...
    ret = 1;
 err:
    // printf("end construct cke ecdhe\n");
