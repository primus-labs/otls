...
static int tls1_generate_key_block(SSL *s, unsigned char *km, size_t num)
{
    ...
    if (need_mpc()) {
        tls1_prf_key_block_tls(s->session->master_key, s->session->master_key_length, s->s3->client_random, SSL3_RANDOM_SIZE, s->s3->server_random, SSL3_RANDOM_SIZE, km, num);
        return 1;
    }
                             ...
                             unsigned char *out)
{
    ...
    // printf("finish mac hash[%d]==>", hashlen);
    // for (int i = 0; i < hashlen; i++) {
    //     printf("%2x ", hash[i]);
    // }
    // printf("\n");
    if (need_mpc()) {
        int client = memcmp(str, "client", strlen("client")) == 0 ? 1: 0;
        transfer_hash_tls(hash, hashlen);
        tls1_prf_finish_mac_tls(s->session->master_key, s->session->master_key_length, hash, hashlen, out, TLS1_FINISH_MAC_LENGTH, client);
    }
    else if (!tls1_PRF(s, str, slen, hash, hashlen, NULL, 0, NULL, 0, NULL, 0,
                                ...
                                size_t len, size_t *secret_size)
{
    if (s->session->flags & SSL_SESS_FLAG_EXTMS) {
        ...
        if (need_mpc()) {
            transfer_hash_tls(hash, hashlen);
            tls1_prf_master_secret_tls(p, len, hash, hashlen, out, SSL3_MASTER_SECRET_SIZE);
        }
        else if (!tls1_PRF(s,
