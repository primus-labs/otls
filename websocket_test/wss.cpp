...
void SendHandshake(SSL* ssl, const char *request)
{
  ...
  int err = SSL_write(ssl, handshakeMsg, (int)strlen(handshakeMsg));
...
string GetMessage(SSL* ssl) {
    ...
    string result;
    if (!tlsData.empty()) {
        result.insert(result.end(), tlsData.begin(), tlsData.end());
        tlsData.erase(tlsData.begin(), tlsData.end());
        return result;
    }
    ...
    while (continueFlag) {
        ...
        int read = SSL_read(ssl, buf, BUFFER_SIZE);
        if (!read) return 0;
        if (read < 0)
        {
...
void RequestWebSocketHandshake(SSL* ssl) {
    char handshakeMsg[] = "GET / HTTP/1.1\r\n"
      ...
      "Upgrade: websocket\r\n";
    int ret = SSL_write(ssl, handshakeMsg, strlen(handshakeMsg));
    if (ret < 0) 
        EXIT_THREAD(0);
}
