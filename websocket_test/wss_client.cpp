...
void run_client() {
    ...
    RequestWebSocketHandshake(ssl);
    CheckWebSocketHandshake(ssl);
    ...
    while (1) {
        ...
        string tlsbuf = GenWebSocketMessage(buf, strlen(buf));
        int len = SSL_write(ssl, tlsbuf.data(), tlsbuf.size());
        printf("client => send %d %s\n", len, buf);

        // len = recv(fd, buf, sizeof(buf), 0);
        // len = SSL_read(ssl, buf, sizeof(buf));
        string s = GetMessage(ssl);
